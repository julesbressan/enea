...
def replace_unknown_to_nan_and_clean_spaces(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for column_name in df.columns:
        ...
        if pd.api.types.is_numeric_dtype(serie.dtype):
            ...
            regex = (
                r"^\s*(" + re.escape(FEATURES_UNKNOWN_VALUES[column_name]) + r")*\s*$"
            )
...
def reduce_nb_categories_for_categorical_feature(
    ...
    for feature in df.columns:
        ...
        if pd.api.types.is_object_dtype(serie.dtype):
            ...
            selected_categories = cumulated_category_weights[
                cumulated_category_weights <= 0.95
            ]

            # We replace if there is more than 10 features and if we can cover more than 95% of the lines with just some features
            if (
                len(selected_categories) < len(serie.value_counts())
                and len(serie.value_counts()) >= 10
            ):
...
def check_and_set_keys(df: pd.DataFrame, filename: str,) -> pd.DataFrame:
    ...
    if ratio_of_duplicates_on_key == 0:
        ...
        logging.error(
            f"{keys} isn't the key of df, too many duplicates: {ratio_of_duplicates_on_key} ratio"
        )
        raise ValueError()
...
def remove_sparse_columns(df: pd.DataFrame, filename: str) -> pd.DataFrame:
    sparsity_columns = {
        column: df[column].isna().sum() / len(df) for column in df.columns
    }
    sparse_columns = [
        column for column in sparsity_columns.keys() if sparsity_columns[column] > 0.50
    ]
...
def process_generic_file_data():
    ...
    for filename in SELECTED_FILES_FEATURES.keys():
        ...
        df, modified_columns = reduce_nb_categories_for_categorical_feature(
            df, filename
        )
...
def merge_file_data(processed_file_data):
    ...
    dfs = [
        filedata.df
        for filedata in processed_file_data.values()
        if filedata.granularity == ["ACCIDENT_NO"]
    ]

    df_merged = reduce(
        lambda left, right: pd.merge(left, right, on=["ACCIDENT_NO"], how="outer"), dfs
    )
    ...
    df_merged = pd.merge(
        person_df, df_merged, on=["ACCIDENT_NO", "VEHICLE_ID"], how="outer"
    )

    return df_merged


# Some specific treatment for that feature
# %%
def categorise_helmet_belt_worn(df):
    df = df.astype({"HELMET_BELT_WORN": int})
    conversion = {
        1: "security_worn",
        3: "security_worn",
        6: "security_worn",
    ...
    df.loc[:, "HELMET_BELT_WORN"] = df.HELMET_BELT_WORN.apply(lambda x: conversion[x])
    return df
...
def main():
    ...
    merged = categorise_helmet_belt_worn(merged)
    # putting labels at the end
    merged = merged[
        [column for column in merged.columns if column != "INJ_LEVEL"] + ["INJ_LEVEL"]
    ]
